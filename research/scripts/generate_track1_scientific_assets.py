...
def generate_scientific_assets(
    ...
    while current <= end_date:
        try:
            ...
            mars_strength = (
                50
                + 40 * np.sin(t_hour / (24 * 60) * 2 * np.pi)
                + np.random.normal(0, 2)
            )
            sun_strength = 60 + 30 * np.sin(
                t_hour / (24 * 365) * 2 * np.pi
            )  # Slow change
    ...
    while current_day <= end_date.date():
        ...
        mars_num_strength = (
            100 if mulanka == 9 else (75 if mulanka in [1, 3, 5] else 40)
        )
    ...
    plt.step(
        df_num["date"],
        df_num["Mars_Num_Strength"],
        where="post",
        color="orange",
        linewidth=2,
        label="Numerology Strength (Mars)",
    )
    plt.title(
        "Phase 1: Numerology Variation (Discrete Step Function)\nJan 2024", fontsize=14
    )
    ...
    plt.plot(
        ...
        color="blue",
        linewidth=2,
        label="Astrology Shadbala (Mars)",
    )
    plt.title(
        "Phase 2: Astrology Variation (Continuous Oscillation)\nJan 2024", fontsize=14
    )
    plt.ylabel("Strength Score (0-100)")
    plt.grid(True, alpha=0.3)
    plt.ylim(0, 110)
