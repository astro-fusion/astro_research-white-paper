...
class PlanetaryStrengthVisualizer:
    ...
    def _load_results(self) -> Dict:
        ...
        try:
            ...
            print(
                f"Warning: {self.research_results_file} not found. Using default data."
            )
    ...
    def generate_time_series_data(self, days: int = 365) -> pd.DataFrame:
        ...
        """
        birth_date = datetime.strptime(self.results["birth_data"]["date"], "%Y-%m-%d")
    ...
    def plot_planetary_strength_timeline(
        self, use_plotly: bool = True, save_path: str = None
    ) -> None:
        """
        ...
        if use_plotly and PLOTLY_AVAILABLE:
            ...
            fig.add_hrect(
                y0=75,
                y1=100,
                annotation_text="Excellent Support",
                fillcolor="green",
                opacity=0.1,
                layer="below",
            )
            ...
            for planet in planets:
                ax.plot(
                    ...
                    df[planet],
                    label=planet,
                    color=colors.get(planet, "#1f77b4"),
                    linewidth=2,
                )

            # Add support zones
            ax.fill_between(
                df["date"], 75, 100, alpha=0.1, color="green", label="Excellent Support"
            )
            ax.fill_between(
                df["date"], 50, 75, alpha=0.1, color="blue", label="Good Support"
            )
            ax.fill_between(
                df["date"], 25, 50, alpha=0.1, color="orange", label="Weak Support"
            )
            ax.fill_between(
                df["date"], 0, 25, alpha=0.1, color="red", label="Poor Support"
            )

            ax.set_title(
                "Planetary Strength Timeline (1 Year)", fontsize=16, fontweight="bold"
            )
    ...
    def plot_numerology_vs_astrology_comparison(
        ...
        if use_plotly and PLOTLY_AVAILABLE:
            ...
            ax.plot(
                df["date"],
                df[mulanka_planet],
                label=f"Mulanka Planet ({mulanka_planet})",
                color="#FF4500",
                linewidth=3,
            )
            ax.plot(
                df["date"],
                df[bhagyanka_planet],
                label=f"Bhagyanka Planet ({bhagyanka_planet})",
                color="#4169E1",
                linewidth=3,
            )
            ax.plot(
                df["date"],
                df["numerology_mulanka_influence"],
                label="Mulanka Numerical Influence",
                color="#FF4500",
                linewidth=2,
                linestyle="--",
            )
            ax.plot(
                df["date"],
                df["numerology_bhagyanka_influence"],
                label="Bhagyanka Numerical Influence",
                color="#4169E1",
                linewidth=2,
                linestyle="--",
            )
            ...
            ax.set_title(
                "Numerology vs Vedic Astrology: Daily Strength Comparison",
                fontsize=16,
                fontweight="bold",
            )
            ax.set_xlabel("Date", fontsize=12)
    ...
    def plot_daily_numerology_changes(
        ...
        start_date = datetime.strptime(self.results["birth_data"]["date"], "%Y-%m-%d")
        ...
        if use_plotly and PLOTLY_AVAILABLE:
            ...
            ax.plot(
                dates,
                mulankas,
                label="Daily Mulanka (Birth Number)",
                color="#FF4500",
                linewidth=2,
                marker="o",
                markersize=6,
            ...
            ax.set_title(
                "Daily Mulanka (Birth Number) Changes", fontsize=16, fontweight="bold"
            )
            ax.set_xlabel("Date", fontsize=12)
...
def main():
    ...
    visualizer.plot_planetary_strength_timeline(
        use_plotly=True, save_path="planetary_strength_timeline.html"
    )

    print("\n2. Generating Numerology vs Astrology Comparison...")
    visualizer.plot_numerology_vs_astrology_comparison(
        use_plotly=True, save_path="numerology_vs_astrology_comparison.html"
    )

    print("\n3. Generating Daily Numerology Changes...")
    visualizer.plot_daily_numerology_changes(
        use_plotly=True, save_path="daily_numerology_changes.html"
    )
