...
class EarthquakeAnalysisTester:
    ...
    def test_load_sample_data(self) -> bool:
        ...
        try:
            ...
            data = fetcher.fetch_earthquakes(
                start_date="2020-01-01", end_date="2020-12-31", use_usgs_api=False
            )
    ...
    def test_data_validation(self) -> bool:
        ...
        try:
            ...
            checks = {
                ...
                "date_format": all(
                    len(eq["date"]) == 10 for eq in processed
                ),  # YYYY-MM-DD
                "coordinates_valid": all(
                    -180 <= eq["longitude"] <= 180 and -90 <= eq["latitude"] <= 90
                    for eq in processed
                ),
    ...
    def run_all_tests(self) -> dict:
        ...
        self._log(
            f"Success Rate: {percentage:.1f}%", "PASS" if percentage == 100 else "WARN"
        )
