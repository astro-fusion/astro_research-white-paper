...
class EarthquakeRigorAnalysis:
    ...
    def generate_rigor_matrix(self, start_date, end_date):
        ...
        for d in dates:
            ...
            for p in self.planets_to_test:
                ...
                row[f"strength_{p.name}"] = scores.get(
                    disp_name, scores.get(p.name, 0.0)
                )

            # Add earthquake counts for this day
            evs = self.df_eq[
                (self.df_eq["dt"].dt.date == d.date())
                & (self.df_eq["magnitude"] >= 6.0)
            ]
    ...
    def run(self):
        ...
        nu_sort = np.interp(
            tau_sort, molchan_mars["tau"][::-1], molchan_mars["nu"][::-1]
        )
...
"""
        report_path = os.path.join(
            ROOT_DIR, "use_cases/earthquake/scripts/earthquake_rigor_report.md"
        )
...
if __name__ == "__main__":
    DATA_PATH = os.path.join(
        ROOT_DIR, "use_cases/earthquake/data/usgs_real_data_phase7.json"
    )
