...
def perform_spectral_analysis(planet=Planet.MARS):
    ...
    df = ast_df.merge(
        num_df_daily[["date_only", f"numerology_{planet.name}"]],
        on="date_only",
        how="left",
    )
    ...
    axes[1].plot(
        xf[freq_mask],
        ast_psd[freq_mask],
        label="Astrology Spectral Power",
        color="#1f77b4",
    )
    axes[1].plot(
        xf[freq_mask],
        num_psd[freq_mask],
        label="Numerology Spectral Power",
        color="#ff7f0e",
        alpha=0.7,
    )
