...
ref_birth_chart = calculate_chart(
    ref_chart.datetime, ref_chart.latitude, ref_chart.longitude
)
ref_moon_long = ref_birth_chart.planets["Moon"]["longitude"]
ref_periods = compute_vimshottari_periods(
    ref_chart.datetime, ref_moon_long, total_years=120
)
...
for d in prices["date"].dt.date:
    ...
    row["global_vimshottari_lord"] = get_dasha_lord(
        ref_periods, dt, ref_chart.datetime.tzinfo
    )
