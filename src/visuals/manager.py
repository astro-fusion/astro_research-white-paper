...
class VisualsManager:
    ...
    async def generate_diagrams(self, requests: List[DiagramRequest]):
        """Generates multiple diagrams in parallel."""
        tasks = []
        for req in requests:
...
async def run_visuals_generation(output_dir: str):
    ...
    requests = [
        ...
        DiagramRequest(
            ...
            3. Statistical Analysis: Running Granger Causality, Lomb-Scargle Periodograms, and Molchan trajectories.
            The final output is a rendered Quarto research report.
            """,
            intent="A flow diagram showing the data processing pipeline from raw data to final analysis.",
        ...
        DiagramRequest(
            ...
            - src.visuals: Diagram generation system with plugins for PaperBanana, Mermaid, LaTeX.
            - scripts/generate: CLI entry points.
            Data flows from src.data -> src.models -> src.visuals -> reports/.
            """,
            intent="A class diagram or component diagram showing the module dependencies.",
        ...
        DiagramRequest(
            context="""
            We model the relationship between Gold Volatility (sigma_t) and Planetary Speed (v_t) using a linear regression with seasonal components.
            Formula: sigma_t = alpha + beta * v_t + gamma * sin(2*pi*t/P) + epsilon_t
            """,
            intent="A visual representation of the regression model equation or a conceptual plot of a sine wave fitted to data points.",
