...
# Paths
PROCESSED_DATA_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'processed', 'aligned_planetary_data.csv')
ARTIFACTS_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'reports', 'artifacts')
...
def generate_stationarity_table(df: pd.DataFrame):
    ...
    variables.extend([c for c in df.columns if '_Sin' in c or '_Cos' in c][:4]) # Limit to a few for brevity
...
def generate_granger_stats(df: pd.DataFrame):
    ...
    for planet in planets:
        ...
        res = run_granger_causality(df, 'Gold_Log_Ret', predictors, maxlag=5, bonferroni_n=len(planets))
        results[planet] = res
...
def generate_molchan_plot(df: pd.DataFrame):
    ...
    plt.plot(molchan_df['Tau'], molchan_df['Nu'], marker='o', markersize=3, label='Mars Speed Model')
...
def generate_monte_carlo_plot(df: pd.DataFrame):
    ...
    res = run_monte_carlo_permutation_test(df, target_col, exog_cols, n_permutations=1000)
    ...
    plt.hist(permuted_scores, bins=50, color='lightgray', edgecolor='gray', alpha=0.7, label='Random Permutations')
    
    # Plot Actual Score
    plt.axvline(actual_score, color='red', linestyle='-', linewidth=2, label=f'Actual (p={p_value:.3f})')
