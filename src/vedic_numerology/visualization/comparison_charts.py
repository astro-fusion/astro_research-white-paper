...
def plot_mulanka_vs_bhagyanka(mulanka_score: float, bhagyanka_score: float,
    ...
    if use_plotly and PLOTLY_AVAILABLE:
        return _plot_comparison_plotly(mulanka_score, bhagyanka_score,
                                     mulanka_planet, bhagyanka_planet)
    else:
        return _plot_comparison_matplotlib(mulanka_score, bhagyanka_score,
                                         mulanka_planet, bhagyanka_planet)


def plot_natal_strength_comparison(chart: Any, mulanka_planet: Planet,
    ...
    factors = _calculate_strength_factors(mulanka_data, bhagyanka_data,
                                        mulanka_planet, bhagyanka_planet)

    if use_plotly and PLOTLY_AVAILABLE:
        return _plot_strength_factors_plotly(factors, mulanka_planet, bhagyanka_planet)
    else:
        return _plot_strength_factors_matplotlib(factors, mulanka_planet, bhagyanka_planet)


def _plot_comparison_plotly(mulanka_score: float, bhagyanka_score: float,
                           mulanka_planet: Optional[Planet],
                           bhagyanka_planet: Optional[Planet]) -> go.Figure:
    """
    ...
    fig.add_hline(y=75, line_dash="dash", line_color="green",
                 annotation_text="Strong Support", annotation_position="top right")
    fig.add_hline(y=50, line_dash="dot", line_color="orange",
                 annotation_text="Neutral", annotation_position="bottom right")
    fig.add_hline(y=25, line_dash="dash", line_color="red",
                 annotation_text="Weak Support", annotation_position="bottom right")
...
def _plot_comparison_matplotlib(mulanka_score: float, bhagyanka_score: float,
                               mulanka_planet: Optional[Planet],
                               bhagyanka_planet: Optional[Planet]) -> plt.Axes:
    """
    ...
    # Add reference lines
    ax.axhline(y=75, color='green', linestyle='--', alpha=0.7, label='Strong Support (75+)')
...
def _calculate_strength_factors(mulanka_data: Dict, bhagyanka_data: Dict,
                              mulanka_planet: Planet, bhagyanka_planet: Planet) -> Dict[str, Dict]:
    """
    ...
    """
    factors = {
        'Position': {'mulanka': 0, 'bhagyanka': 0, 'description': 'Essential dignity (exaltation, etc.)'},
        'Direction': {'mulanka': 0, 'bhagyanka': 0, 'description': 'Directional strength ( Kendra positions)'},
        'Temporal': {'mulanka': 0, 'bhagyanka': 0, 'description': 'Time-based strength (day/night)'},
        'Motion': {'mulanka': 0, 'bhagyanka': 0, 'description': 'Motion strength (retrograde bonus)'},
        'Combustion': {'mulanka': 0, 'bhagyanka': 0, 'description': 'Combustion penalty (inverse)'}
    }
    ...
    factors['Position']['mulanka'] = _estimate_position_strength(mulanka_planet, mulanka_sign)
    factors['Position']['bhagyanka'] = _estimate_position_strength(bhagyanka_planet, bhagyanka_sign)
    ...
    if is_daytime:
        ...
        factors['Temporal']['bhagyanka'] = 80 if bhagyanka_planet in night_planets else 40
...
def _estimate_position_strength(planet: Planet, sign: int) -> float:
    ...
    """
    from ..dignity.exaltation_matrix import is_in_exaltation, is_in_moolatrikona, is_in_own_sign
...
def _plot_strength_factors_plotly(factors: Dict[str, Dict], mulanka_planet: Planet,
                                bhagyanka_planet: Planet) -> go.Figure:
    """
...
def _plot_strength_factors_matplotlib(factors: Dict[str, Dict], mulanka_planet: Planet,
                                    bhagyanka_planet: Planet) -> plt.Axes:
    """
    ...
    # Plot data
    ax.plot(angles, mulanka_values, 'o-', linewidth=2, label=f'Mulanka ({mulanka_planet.name})',
           color=COMPARISON_COLORS['mulanka'])
    ax.fill(angles, mulanka_values, alpha=0.25, color=COMPARISON_COLORS['mulanka'])

    ax.plot(angles, bhagyanka_values, 'o-', linewidth=2, label=f'Bhagyanka ({bhagyanka_planet.name})',
    ...
    # Title and legend
    ax.set_title(f"Strength Factors: {mulanka_planet.name} vs {bhagyanka_planet.name}",
