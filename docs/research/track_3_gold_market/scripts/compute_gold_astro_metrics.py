...
for _, row in catalog.iterrows():
    ...
    if cat in ["major_aspect", "minor_aspect", "vedic_malefic_aspect"]:
        ...
        active = (
            feat[col].values.astype(bool)
            if col in feat.columns
            else np.zeros(len(feat), dtype=bool)
        )
    ...
    for w, dayset in window_sets.items():
        overlap = int(
            sum(1 for i, d in enumerate(feat_days) if active[i] and d in dayset)
        )
...
# Multiple comparison correction (Using shared stats module)
