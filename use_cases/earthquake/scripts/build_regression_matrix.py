...
def build_matrix(start_year: int = 2020, end_year: int = 2023, output_path: str = "regression_matrix.csv"):
    """
    ...
    eq_fetcher = EarthquakeDataFetcher(use_sample_data=True, verbose=False) # Use mock/sample for demo, real in prod
    ...
    real_data_path = Path(__file__).parent.parent.parent / "data" / "usgs_real_data_phase7.json"
    ...
    for i in range(delta.days + 1):
        ...
        # --- Numerology Features ---
        udn = num_engine.calculate_universal_day_number(current_date, preserve_master=True)
        uyn = num_engine.calculate_universal_year_number(current_date.year, preserve_master=True)
