...
def run_validation(matrix_path: str = "regression_matrix.csv", n_shuffles: int = 1000):
    ...
    p_monte_carlo = (n_better + 1) / (
        len(random_deltas) + 1
    )  # Add 1 for the real observation
    ...
    try:
        ...
        plt.hist(
            random_deltas, bins=30, alpha=0.7, color="gray", label="Random Shuffles"
        )
        plt.axvline(
            real_delta_aic,
            color="red",
